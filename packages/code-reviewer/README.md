...
| `COMPANY_LLM_BASE_URL` | LLM 网关 baseUrl |
| `COMPANY_AI_TOKEN` | LLM 网关 token |
| `SIEM_INGEST_URL` | 审计上报地址(可选) |

### 构建镜像

```bash
# 在 flower 根目录
docker build -f packages/code-reviewer/Dockerfile -t yourcompany/code-reviewer:latest .
```
