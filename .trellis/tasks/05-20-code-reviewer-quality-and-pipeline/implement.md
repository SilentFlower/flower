...
**Checklist**:
- [ ] 1.1 `packages/flower-code-reviewer/src/prompts.ts` 追加 6 条新硬约束(逐字抄 `research/comment-style.md` §5;design.md §1.1 列了 key point)
- [ ] 1.2 `prompts.ts` 末尾追加 5 个完整中文模板样例 few-shot(逐字抄 `research/comment-style.md` §6.1-§6.6)
- [ ] 1.3 新增 `packages/flower-code-reviewer/src/comments/` 子目录(或按现有约定),实现 `renderWalkthrough` / `renderInlineComment` / `renderCleanReview`(签名见 design.md §1.2)
- [ ] 1.4 实现 GitLab 版本探测 `detectGitlabVersion` + `supportsAlertBlock`(design.md §1.3),启动期一次性 cache
- [ ] 1.5 `renderWalkthrough` 内集成 alert 块降级分支(`>= 17.10` 用 `> [!caution]`,否则降级 blockquote)
- [ ] 1.6 单测:
  - 行内 4 段式渲染 1 case
...
- [ ] 1.7 typecheck + lint + test 全绿
- [ ] 1.8 临时跑一次 fork sandbox IID 1(用 dev image 或本地 `flower-review --mr-iid 1`),visual check 评论 markdown 在 GitLab 网页渲染正确
...
**Checklist**:
- [ ] 2.1 `packages/flower-tools-gitlab/src/client.ts` 加 `gitlab_get_file_content` 函数(签名见 design.md §2.1)
- [ ] 2.2 `packages/flower-tools-gitlab/src/index.ts` export 第 6 个工具
- [ ] 2.3 `packages/flower-tools-gitlab/src/client.test.ts` 加 3+ case:成功 / 404 / 5xx 重试(参考既有 14 case 模式)
- [ ] 2.4 `packages/flower-code-reviewer/src/` 加 `safeReadFile` wrapper(design.md §2.4):env `FLOWER_MAX_FILE_SIZE` + 二进制后缀跳过
- [ ] 2.5 `prompts.ts` 加第 7 条硬约束(每文件必读;design.md §2.2)
- [ ] 2.6 `packages/flower-code-reviewer/src/run.ts` `scanForBlockers` 扩展 「无依据评论」拦截(design.md §2.3):trace 已拉文件集合 + finalize 时扫 line_comment path 是否 ∉ readFiles
- [ ] 2.7 单测:
...
- [ ] 2.8 typecheck + lint + test 全绿
- [ ] 2.9 fork sandbox IID 2 试跑(本地 dev),verify LLM 实际调用 `gitlab_get_file_content`(看 SIEM trace 或 stdout log)
...
**Checklist**:
- [ ] 3.1 E1:`run.ts` 顶层 try/catch + `isLlmFailure` 判定 + `postWarningComment` 路径(design.md §5.1)
- [ ] 3.2 E1 单测:mock LLM 全失败 → warning 评论 post 1 次 + exit 0
- [ ] 3.3 E2:diff 加载阶段加 churn 排序 + cap(env `FLOWER_MAX_FILES` 默认 50);截断时 `renderWalkthrough` 传 `truncatedFiles`(design.md §5.2)
- [ ] 3.4 E2 单测:51 文件 mock → 取 top 50 + walkthrough 含「⚠️ 本次仅评 50/51」
- [ ] 3.5 E3:新建 `packages/flower-code-reviewer/src/sanitize.ts` `sanitizeQuickActions` 纯函数(design.md §5.3)
- [ ] 3.6 E3:`gitlab_post_comment` / `gitlab_post_line_comment` 调用点 wrapper 接入 sanitize
- [ ] 3.7 E3 单测:`sanitize.test.ts` 12+ quick action case + 普通行不动 + 多行混合
- [ ] 3.8 typecheck + lint + test 全绿
...
**Checklist**:
- [ ] 6.1 单测 + lint + typecheck 全绿(全部 4 个 package,Phase 3 已 ✓ 320+ cases)
- [ ] 6.2 spec 沉淀(本次跨包发现):
