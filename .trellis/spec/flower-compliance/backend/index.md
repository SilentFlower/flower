...
`flower-compliance` 是 pi 扩展库,内部实现集中在两个模块:

- `src/audit.ts`:封装 `sendAudit(record)`,异步 POST 到 SIEM 端点
- `src/index.ts`:注册 `ci-readonly` / `production-readonly` 合规拦截;CI 模式禁写工具,并按 bash 白名单控制命令

本目录(`backend/`)关心审计上报与工具调用拦截:错误处理、日志、配置、副作用边界。
